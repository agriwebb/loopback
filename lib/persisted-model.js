...
module.exports = function(registry) {
    ...
    if (options.trackChanges || options.enableRemoteReplication) {
      ...
      setRemoting(PersistedModel, 'changes', {
        ...
        accepts: [
          ...
          {arg: 'tenant', type: 'any', description: 'Only include changes that match this tenant'},
        ],
  ...
  function tryReplicate(sourceModel, targetModel, since, options, callback) {
    ...
    function getSourceChanges(cb) {
      chunk.downloadInChunks(options.filter, function(filter, pagingCallback) {
    ...
    function getDiffFromTarget(sourceChanges, cb) {
      chunk.processInChunks(sourceChanges, sourceModel.getChunkSize(), function(smallArray, chunkCallback) {
    ...
    function createSourceUpdates(_diff, cb) {
      ...
      if (diff && diff.deltas && diff.deltas.length) {
        ...
        chunk.processInChunks(diff.deltas, sourceModel.getChunkSize(), function(smallArray, chunkCallback) {
    ...
    function bulkUpdate(_updates, cb) {
      ...
      updates = _updates;
      chunk.processInChunks(updates, sourceModel.getChunkSize(), function(smallArray, chunkCallback) {
  ...
  PersistedModel.bulkUpdate = function(updates, options, callback) {
    ...
    options = options || {};
    options.deferAfterSave = Boolean(Model.settings.deferAfterSave);
    ...
    buildLookupOfAffectedModelData(Model, updates, function(err, currentMap) {
      ...
      async.parallel(tasks, function(err) {
        ...
        if (options.afterSaveContextMap) {
          afterSaveTasks = updates
            .filter(function (u) {
              return !!options.afterSaveContextMap[u.change.modelId];
            })
            .map(function(u) {
              return function executeAfterSave(cb) {
                var context = options.afterSaveContextMap[u.change.modelId];
                context.Model.notifyObserversOf('after save', context, function(err) {
                  // Continue through all after saves even if there was an error
                  cb(null, err);
                });
              };
            });
        }
        async.series(afterSaveTasks, function (_unusedError, afterSaveErrors) {
          if (Array.isArray(afterSaveErrors)) {
            afterSaveErrors = afterSaveErrors.filter(function(err) {
              return err !== undefined && err !== null;
            });
            if (afterSaveErrors.length > 0) {
              err = afterSaveErrors[0];
              err.statusCode = 500;
              err.afterSaveErrors = afterSaveErrors;
              return callback(err);
            }
          }
          if (conflicts.length) {
  ...
  function applyUpdate(Model, id, current, data, change, conflicts, options, cb) {
    ...
    Model.upsert(data, options, function(err, result) {
      if (err) return cb(err);
